...
/// practice puzzles don't need to reproduce across devices.
///
...
public enum PracticeGenerator {
    public struct Puzzle: Sendable, Hashable {
        ...
        public let level: PracticeLevel
    ...
    /// `PracticeGeneratorTests.testEveryLevelTerminates`.
    ...
    public static func generate(
        ...
        // `PracticeGeneratorTests.testEveryLevelTerminates`. If some future change to the level
        ...
        assertionFailure("PracticeGenerator exhausted \(maxAttemptsPerLevel) attempts for level \(level) -- falling back")
    ...
    private static func attemptRepeatedly(level: PracticeLevel, diceCount: Int, diceSides: Int) -> Puzzle? {
    ...
    private static func attempt(faces: [Int], level: PracticeLevel) -> Puzzle? {
        let tier = level.tier
        ...
        // `DieValue.practiceVariants`), that would have silently meant "every Roots-level roll
        ...
        for (index, face) in faces.enumerated() {
            let variants = DieValue.practiceVariants(
                base: face,
        ...
        // casing Practice to mean something else.
